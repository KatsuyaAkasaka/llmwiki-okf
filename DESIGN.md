...
2. 見つからなければ**明示指定** — CLI はパス引数(`update <dir>` / `lint <bundle-dir>`)、
   スキルは会話で指定されたパスを使い、不明ならユーザーに尋ねる(推測しない)

補足: 一時期 `LLMWIKI_WIKI_DIR` 環境変数によるフォールバックを導入したが撤去した。
環境変数はシェル再起動まで反映されず挙動が読みにくいこと、明示パス指定で十分なことが
理由(raw ディレクトリの `LLMWIKI_RAW_DIR` は書き込み先ではなく置き場の指定なので存続)。
...
ユーザー所有物なので決して書き換えない。冪等。対象は引数 > cwd 上方探索で解決。
